...
public class MyOrderItemItemAdapter extends BaseAdapter {
	...
	private List<OrderItemVO2> _list;

	public MyOrderItemItemAdapter(Context context, List<OrderItemVO2> list) {
		super();
		this._context = context;
		this._list = list;
	}

	public List<OrderItemVO2> get_list() {
		return _list;
	}

	public void set_list(List<OrderItemVO2> _list) {
		this._list = _list;
	}
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		OrderItemVO2 oneitem = (OrderItemVO2) getItem(position);
		Holder holder;
		if (null == convertView) {
			convertView = View.inflate(_context, R.layout.adapter_myorder_item_item,
					null);
			holder = new Holder();
